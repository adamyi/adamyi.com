...
const RE_REGION = /^{#\s?region:\s?(.*?)\s?#}\s?\n/m;
const RE_VERTICAL = /^{#\s?vertical:\s?(.*?)\s?#}\s?\n/m;
const RE_FEATURED_DATE = /^{#\s?featured_date:\s?(.*?)\s?#}\s?\n/m;

const RE_UPDATED = /^{#\s?updated_on:\s?(.*?)\s?#}\s?\n/m;
const RE_PUBLISHED = /^{#\s?published_on:\s?(.*?)\s?#}\s?\n/m;

const RE_IMAGE = /^{#\s?featured_image:\s?(.*?)\s?#}\s?\n/m;
const RE_IMAGE_SQUARE = /^{#\s?featured_image_square:\s?(.*?)\s?#}\s?\n/m;
const RE_BLINK_COMPONENTS = /^{#\s?blink_components:\s?(.*?)\s?#}\s?\n/m;
const RE_TAGS = /^{#\s?tags:\s?(.*?)\s?#}\s?\n/m;
const RE_SNIPPET = /^{#\s?featured_snippet:\s?(.*?)\s?#}\s?\n/m;
...
const RE_PODCAST = /^{#\s?podcast_audio: (.*?) #}\s?\n/m;
const RE_PODCAST_DURATION = /^{#\s?podcast_duration: (.*?)\s?#}\s?\n/m;
const RE_PODCAST_SUBTITLE = /^{#\s?podcast_subtitle: (.*?)\s?#}\s?\n/m;
const RE_PODCAST_SIZE = /^{#\s?podcast_fileSize: (.*?)\s?#}\s?\n/m;
...
const RE_IGNORE_FILE = /{#\s?ignore_file.*?#}/;
const RE_IS_INCLUDE = /{#\s?include\s?#}/;
const RE_IS_MD_INCLUDE = /{#\s?md_include\s?#}/;
const RE_USES_TEMPLATE = /{#\s?uses_template\s?#}/;
const RE_AUTO_GENERATED = /{#\s?auto_generated\s?#}/;
const RE_DEVSITE_TRANSLATION = /{#\s?devsite_translation\s?#}/;
